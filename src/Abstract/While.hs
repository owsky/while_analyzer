  ...
  absWhileSemantics' inv thresholds widenDelay descendSteps (WhileDo c b e) s =
    let
      ...
      f (x, invAcc) =
        ...
        in (s `lub` newState, invAcc `Map.union` newInv)
...
-- type InvMap a = Map.Map Bexp (NonRelational Text a)
...
--   While -> -- input While statement
--   NonRelational Text a -> -- input abstract state
--   (NonRelational Text a, InvMap a) -- tuple of output abstract state and set of loop invariants
-- absWhileSemantics = absWhileSemantics Map.empty -- tail recursive subfunction
--  where
--   -- evaluating a statements against a Bottom state will always result in a Bottom state
--   absWhileSemantics inv _ _ _ _ Bottom = (bottom, inv)
...
--       -- functional
--       f (x, invAcc) =
--         let (x', bodyInvs) = (absWhileSemantics inv thresholds widenDelay descendSteps e . absBexpSemantics b) x
--         in (s `lub` x', invAcc `Map.union` bodyInvs)
...
--       -- find the best least fixpoint approximation by narrowing the postfixpoint for a fixed amount of
--       -- times in order to ensure convergence
--       (lfp, lfpInvs) = iterateN f postFix descendSteps
--     in
--       -- lastly filter the lfp state with the negated guard of the loop, and keep track of the
--       -- lfp as it is the loop's invariant
--       (absBexpSemantics (BexpUnary Not b) lfp, lfpInvs `Map.union` Map.insert b lfp inv)
--    where
--     -- Kleeni iterations with optional widening
--     iterateW g (x, invAcc) (Just 0) =
--       let (x', invAcc') = g (x, invAcc)
--           xWide = widening thresholds x x'
--       in if xWide == x then (x, invAcc) else iterateW g (x', invAcc') (Just 0)
--     iterateW g (x, invAcc) k =
--       let (x', invAcc') = g (x, invAcc)
--       in if x' == x then (x, invAcc) else iterateW g (x', invAcc') (decrMaybe k)
...
--     iterateN g (x, invAcc) n =
--       let (x', invAcc') = g (x, invAcc)
--           xNarrow = x `narrowing` x'
--       in if xNarrow == x then (x, invAcc) else iterateN g (x', invAcc') (decrMaybe n)